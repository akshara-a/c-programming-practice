...
struct Bankacc {
    ...
    char accHolder[100];
    float Bal;
    int isAccountCreated; // Flag to check if an account is created
};
...
void createacc(struct Bankacc *acc) {
    ...
    acc->Bal = 0.0;
    acc->isAccountCreated = 1; // Set the flag to indicate account creation
    printf("Account created successfully!\n");
}

void deposit(struct Bankacc *acc) {
    if (!acc->isAccountCreated) {
        printf("No account holder found. Please create an account first.\n");
        return;
    }
...
int main() {
    ...
    acc.isAccountCreated = 0; // Initialize the flag
